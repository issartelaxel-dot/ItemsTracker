...
const ALLOW_SAME_ORIGIN_API_FALLBACK = String(import.meta.env.VITE_ALLOW_SAME_ORIGIN_API_FALLBACK ?? '')
  .trim()
  .toLowerCase() === 'true'
const AUTO_SAVE_INTERVAL_MS = 40_000
...
function App() {
  ...
  const [itemDetailTab, setItemDetailTab] = useState<ItemDetailTab>('tracking')
...
function getPasswordStrengthMeta(password: string) {
  ...
  const quizSessionSummaryContext = useMemo(() => {
    const modeLabel = quizSessionMode === 'quiz' ? 'QCM' : 'Flashcards'
    const cardCount = quizSessionMetrics.cards.length
    const cardLabel = `${cardCount} carte${cardCount > 1 ? 's' : ''}`
    const scopeLabel = quizSessionScopeLabel.replace(/^Révision\s+/i, '')
    return `${modeLabel} · ${cardLabel} · ${scopeLabel}`
  }, [quizSessionMetrics.cards.length, quizSessionMode, quizSessionScopeLabel])
              ...
              <div className="meta-grid item-detail-stat-grid">
                <div>
                  <span className="item-detail-stat-icon" aria-hidden="true">
                    <ItemDetailStatIcon name="reviews" />
                  </span>
                ...
                >
                  <span className="item-detail-stat-icon" aria-hidden="true">
                    <ItemDetailFeelingIcon mastery={effectiveSelectedItem.tracking.itemMastery} />
                  </span>
                  <select
                ...
                <div>
                  <span className="item-detail-stat-icon" aria-hidden="true">
                    <ItemDetailStatIcon name="resources" />
                  </span>
                ...
                <div className="item-detail-main-column">
                    <label className="block-label item-detail-card item-detail-upcoming-card item-detail-tab-panel item-detail-tab-panel-tracking">
              ...
              <>
                ...
                <details className="flashcard-editor-notes">
                  ...
                  <textarea
                    ...
                    placeholder="Ajoutez ici des notes personnelles sur cette carte..."
                    onChange={(event) =>
                      updateQuizCard(quizItem.itemNumber, activeQuizCard.id, { personalNotes: event.target.value })
                    }
                  />
                </details>
                ...
                {flashGeneratorStep === 2 ? (
                  <div className="flash-generator-step">
                    ...
                    <div className="flashcards-mode-row">
                      ...
                      <button type="button" className={`ghost-btn ${flashSelectedFeelings.includes('medium') ? 'active' : ''}`} onClick={() => toggleFlashFeeling('medium')}>
                        {FLASH_FEELING_LABELS.medium}
                      </button>
